...
DECLARE
    ...
    v_id_int_comprobante_rep	integer;
...
BEGIN
     ...
     IF p_codigo_estado = 'pendiente' THEN
                ...
                IF (v_sincronizar = 'true') THEN
               	  --(may)29-08-2019 comentar conexiones con endesis
                ...
                ELSE
                --(may) SI EL COMPROBANTE ES DE RENDICION

                	SELECT cd.tipo_rendicion
                    INTO v_tipo_rendicion
                    FROM  cd.tcuenta_doc cd
                    WHERE cd.id_cuenta_doc = v_reg_cuenta_doc.id_cuenta_doc;

                    IF(v_tipo_rendicion != 'rendir_reponer' )THEN
                    --(may) SI ES DE TIPO  rendir
                    ...
                    --(may) SI ES DE TIPO  rendir/reponer
						...
						*/
                             ...
                             update  cd.tcuenta_doc   p set
                              id_int_comprobante = v_id_int_comprobante,
                              id_int_comprobante_reposicion = v_id_int_comprobante_rep
                              where id_proceso_wf = p_id_proceso_wf;
                    END IF;
