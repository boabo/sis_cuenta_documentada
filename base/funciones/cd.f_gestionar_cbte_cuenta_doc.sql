...
BEGIN
	...
	v_nombre_funcion = 'cd.f_gestionar_cbte_cuenta_doc';
		...
		--(may) saca la plantilla del comprobante con reposicion 39
    	SELECT cb.id_plantilla_comprobante
        INTO v_id_plantilla_comprobante
        FROM  conta.tint_comprobante cb
        WHERE  cb.id_int_comprobante = p_id_int_comprobante;

 		--(may)	si es de la plantilla 39 con reposicion de la rendicion saca del id_int_comprobante_reposicion para recuperar el id_cuenta_doc
 		IF (v_id_plantilla_comprobante = 39) THEN
        ...
        -- 1) con el id_comprobante identificar el plan de pago
                ...
                where  pc.id_int_comprobante_reposicion = p_id_int_comprobante;
    ...
    --------------------------------------------------------
         ...
         --(may)los de reposicion de la rendicion no controla el flujo
         IF (v_id_plantilla_comprobante != 39) THEN
         ...
         END IF;
            ...
            IF v_registros.sw_solicitud = 'no' and v_registros.id_cuenta_doc_fk is not null  THEN
                  ...
                  update cd.tcuenta_doc pc  set
                     importe_total_rendido = COALESCE( v_total_rendido, 0)
